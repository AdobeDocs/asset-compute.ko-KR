...
이 명령은 아래 설명된 대로 프로젝트의 Asset compute 응용 프로그램 작업에 대한 사용자 지정 단위 테스트 프레임워크를 실행합니다. 의 구성을 통해 연결됩니다. `package.json` 파일. Jest와 같은 JavaScript 단위 테스트가 있을 수도 있습니다. 다음 `aio app test` 둘 다 실행합니다.
...
내부 테스트가 필요합니다. `test` 폴더의 루트 수준이 표시됩니다. 각 애플리케이션의 테스트 사례는 경로에 있어야 합니다. `test/asset-compute/<worker-name>`, 각 테스트 사례에 대해 하나의 폴더 포함:
...
다음 `build` Adobe Developer App Builder 앱의 루트에 있는 디렉터리에는 사용자 지정 애플리케이션의 자세한 테스트 결과와 로그가 들어 있습니다. 이러한 세부 사항은 의 출력에도 표시됩니다. `aio app test` 명령입니다.

### 모의 외부 서비스 {#mock-external-services}

다음을 만들어 작업 내에서 외부 서비스 호출을 시뮬레이션할 수 있습니다. `mock-<HOST_NAME>.json` 테스트 시나리오용 파일로, HOST_NAME은 모방하려는 특정 호스트입니다. 사용 사례의 예는 S3에 대해 별도의 호출을 수행하는 애플리케이션입니다. 새 테스트 구조는 다음과 같습니다.
...
예 `worker-animal-pictures` 다음 포함: [모의 파일](https://github.com/adobe/asset-compute-example-workers/blob/master/projects/worker-animal-pictures/test/asset-compute/worker-animal-pictures/simple-test/mock-upload.wikimedia.org.json) 와 상호 작용하는 위키미디어 서비스용입니다.

#### 여러 테스트 사례에서 파일 공유 {#share-files-across-test-cases}

Adobe 를 공유하는 경우 상대 심볼릭 링크를 사용하는 것이 좋습니다. `file.*`, `params.json` 또는 `validate` 여러 테스트에 걸친 스크립트. Git에서 지원됩니다. 다른 이름이 있을 수 있으므로 공유 파일에 고유한 이름을 지정해야 합니다. 아래 예에서 테스트는 몇 개의 공유 파일과 자체 파일을 혼합하여 일치시키고 있습니다.
...
다음 `aio` 즉시 사용할 수 있도록 이러한 많은 단계를 자동화합니다. 의 응용 프로그램 디버깅 섹션으로 이동합니다. [Adobe Developer App Builder 설명서](https://developer.adobe.com/app-builder/docs/getting_started/first_app). 현재 아래 단계에는 해결 방법이 포함되어 있습니다.
...
1. JSON 파일에서 사용자 설정에 추가합니다. 이전 Visual Studio Code Debugger를 계속 사용합니다. 새 것은 [일부 문제](https://github.com/apache/openwhisk-wskdebug/issues/74) wskdebug 사용: `"debug.javascript.usePreview": false`.
1. 을 통해 열려 있는 앱의 모든 인스턴스 닫기 `aio app run`.
1. 를 사용하여 최신 코드 배포 `aio app deploy`.
1. 다음을 사용하여 Asset compute Devtool만 실행 `aio asset-compute devtool`. 열어 두십시오.
1. Visual Studio 코드 편집기에서 다음 디버그 구성을 `launch.json`:
...
1. 클릭 **[!UICONTROL 실행]** 개발자 도구에서. Visual Studio 코드 편집기에서 실행 중인 작업을 볼 수 있으며 로그가 표시되기 시작합니다.
