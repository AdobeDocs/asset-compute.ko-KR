...
*그림: Adobe [!DNL Asset Compute Service]을(를) 사용하여 자산을 처리할 때 관련된 단계*
...
[!DNL Asset Compute Service]이(가) 사용자 지정 응용 프로그램에 사용자 지정 응용 프로그램 렌디션 요청을 보냅니다. 제공된 애플리케이션 URL(App Builder의 보안 웹 작업 URL)에 대한 HTTP POST를 사용합니다. 모든 요청은 HTTPS 프로토콜을 사용하여 데이터 보안을 극대화합니다.
...
SDK에서 각 렌디션에 대해 비동기 [렌디션 콜백 함수](https://github.com/adobe/asset-compute-sdk#rendition-callback-for-worker-required)를 호출합니다.
...
렌디션 콜백 매개 변수에 대한 자세한 내용은 [Asset Compute SDK API](https://github.com/adobe/asset-compute-sdk#api-details)를 참조하십시오.

### 렌디션 업로드 {#upload-rendition}

각 렌디션을 만들어 `rendition.path`에서 제공한 경로를 사용하여 파일에 저장하면 [Asset Compute SDK](https://github.com/adobe/asset-compute-sdk#adobe-asset-compute-worker-sdk)에서 각 렌디션을 클라우드 저장소(AWS 또는 Azure)에 업로드합니다. 들어오는 요청에 동일한 애플리케이션 URL을 가리키는 여러 변환이 있는 경우에만 사용자 정의 애플리케이션이 동시에 여러 변환을 가져옵니다. 클라우드 스토리지로의 업로드는 각 렌디션 후 다음 렌디션에 대한 콜백을 실행하기 전에 수행됩니다.

`batchWorker()`에 다른 동작이 있습니다. 모든 렌디션을 처리하고, 모든 렌디션이 처리된 후에만 업로드합니다.

## [!DNL Adobe I/O Events] {#aio-events}

SDK에서 각 렌디션에 대해 Adobe [!DNL I/O Events]을(를) 보냅니다. 이 이벤트는 결과에 따라 `rendition_created` 또는 `rendition_failed` 형식입니다. 자세한 내용은 [Asset Compute 비동기 이벤트](api.md#asynchronous-events)를 참조하십시오.
